...
**Chất (SpiceType)** trong game: `chili`, `lemon`, `avocado` (ứng với Ớt / Chanh / Bơ trong bản tiếng Việt UI).

### 4.2. Total Wild (lá tối thượng / siêu joker)
...
- **Khi đánh Total Wild**: không vào giai đoạn thách — vào **`SUPREME_RESOLVE`** trong vài giây (mọi người **thấy lá thật** + hiệu ứng), rồi tuyên bố được **chấp nhận** như không ai thách. Người chơi được **chọn bất kỳ chất và số 1–10** (bỏ qua khóa chất / leo hạng của vòng); sau đó **chất khóa** và **chuỗi hạng** tiếp tục từ tuyên bố đó.
- Nếu trong tương lai có thách lên lá đã lộ: **Total Wild** vẫn luôn **pass** kiểm tra chất và số (không “bắt bluff” bằng một thách đơn).
...
- **Thách sai**: người **tuyên bố** **thắng** chồng bàn; người thách **rút phạt** **PENALTY_DRAW_COUNT** (**2 lá**) từ chồng rút. Nếu sau khi rút **tay không còn Total Wild** và **dự trữ supreme** còn — có thể **thêm 1 Total Wild** từ dự trữ vào tay (cơ chế “hoàn trả” joker).
  - Nếu người tuyên bố **vừa đánh hết bài**, lá **không phải** Total Wild và **còn chiến tích** trong pool: sau khi nhận xấp bàn họ cũng nhận **1 chiến tích + rút bù 5 lá** (cùng quy tắc mục 8; triển khai trong `applyPenalty`). Phase vẫn là **`PENALTY`** để UI xử lý phạt người thách.
...
### Hết bài tay sau khi đánh (chiến tích & rút bù)

Áp dụng khi lượt đó **kết thúc với tay rỗng** và một trong các cách sau:

1. **Chấp nhận tuyên bố** — không ai thách thành công (claim race hết giờ / mọi người pass), hoặc sau **`SUPREME_RESOLVE`** (Total Wild) rồi `acceptDeclaration`.
2. **Thách sai** — người tuyên bố **thắng** thách (`applyPenalty` với `challengeCorrect === false`).

Điều kiện chi tiết:

- Nếu lá vừa đánh **không phải** Total Wild **và** vẫn còn trophy trong pool:
  - Nhận **1 lá trophy**, tăng `trophyCount`, **rút bù** **REFILL_HAND_SIZE** (**5 lá**).
  - Nhánh **chấp nhận** (và không phải kết thúc ván ngay): phase tạm **`TROPHY_AWARDED`**.
  - Nhánh **thắng thách**: phase là **`PENALTY`** (vẫn trao chiến tích + bù trong state).
- Trường hợp khác (Total Wild hoặc hết trophy): **chỉ rút bù 5 lá** (không thêm trophy).

Logic dùng chung `applyEmptyHandDeclarerTrophyAndRefill` trong engine (gọi từ `acceptDeclaration` và `applyPenalty`).
...
`LOBBY` → `PLAYER_TURN` → `CHALLENGE_PHASE` hoặc **`SUPREME_RESOLVE`** (Total Wild) → (có thể) `REVEAL` → `PENALTY` → `PLAYER_TURN` / `END_GAME`; giữa chừng có `NEXT_TURN`, `TROPHY_AWARDED` tùy nhánh accept.
