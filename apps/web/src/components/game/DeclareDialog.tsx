...
import { Declaration, SpiceType, SPICE_EMOJI, SPICE_LABEL } from '@/lib/types';
...
export function DeclareDialog({ isOpen, onDeclare, onClose, selectedCardId }: DeclareDialogProps) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display text-lg text-foreground text-center flex-1">
            {t('declare.title')}
          </h3>
          <Button
            variant="ghost"
        ...
        {selectedCardId && (
          <div className="mb-4 p-3 bg-muted rounded-lg flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{t('declare.cardSelected')}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          >
            {t('declare.confirm')} {selectedType && selectedNumber ? `${SPICE_EMOJI[selectedType]} ${selectedNumber}` : ''}
          </Button>
        </div>
      </motion.div>
    </motion.div>
