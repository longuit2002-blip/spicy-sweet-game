...
import type { ChallengeResult, Player } from '@/shared/types/game';
    ...
    >
      ...
      >
        ...
        <p className="text-muted-foreground text-sm mb-1">
          {t('declare.preview')}: {SPICE_EMOJI[result.declaredCard.type]} {SPICE_LABEL[result.declaredCard.type]}{' '}
          {result.declaredCard.number}
        </p>
        <p className="text-muted-foreground text-sm mb-4">
          {t('result.realCard')}: {SPICE_EMOJI[result.realCard.type]} {SPICE_LABEL[result.realCard.type]}{' '}
          {result.realCard.number}
        </p>

        {result.wasBluff ? (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4, type: 'spring' }}>
            <p className="font-display text-2xl text-destructive mb-2">
